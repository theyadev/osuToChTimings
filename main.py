...
def convert_to_clone_hero_format(
    ...
    for line in timing_points:
        try:
            ...
            # Log information
            logger.debug(f"{timing}ms: BPM: {bpm}, Signature: {signature}/4 ({ticks} ticks)")
...
def generate_clone_hero_output(ch_timing_lines: List[Tuple[int, int, int, float]]) -> str:
    ...
    """
    output = "// Generated by osu! to Clone Hero Timing Converter\n"
    output += "// https://github.com/theyadev/osuToChTimings\n"
    output += "[SyncTrack]\n{\n"
    for ticks, bpm, signature, _ in ch_timing_lines:
        output += f"  {ticks} = TS {signature}\n"
        output += f"  {ticks} = B {int(bpm)}000\n"
    output += "}\n"
